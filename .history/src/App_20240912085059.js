...
function App() {
  ...
  const [fetchNewCourse, setFetchNewCourse] = useState(false);
  ...
  const fetchCourses = () => {
    fetch('http://127.0.0.1:8000/api/courses/')
      ...
      .then((data) => {
        setCourses(data);  // Update the state with the new courses
      })
      .catch((error) => {
  ...
  const handleCourseCreated = () => {
    fetchCourses();  // Refresh the course list after a course is created
    setFetchNewCourse(false);  // Reset the fetch flag if used
  };
  ...
  const deleteCourse = (id) => {
    ...
    fetch(`http://127.0.0.1:8000/api/courses/${id}/`, {  // Ensure trailing slash
      ...
      .then((response) => {
        if (response.ok) {
          fetchCourses();  // Refresh the course list immediately after deletion
  ...
  useEffect(() => {
    fetchCourses();  // Fetch courses once when the component mounts
    ...
    <div className="App">
      ...
      <div className="container">
        ...
        <section className="section">
          ...
          <CourseForm onCourseCreated={handleCourseCreated} setFetchNewCourse={setFetchNewCourse} />
        </section>
        ...
        <section className="section">
          ...
          <CourseInstanceList
            ...
            year={year}
            semester={semester}
            viewInstanceDetails={setSelectedInstance}
