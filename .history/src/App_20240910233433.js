...
function App() {
  ...
  const deleteInstance = (id) => {
    ...
    console.log("Initial instances:", instances); // Check instances before deletion
    ...
    .then(response => {
        if (response.ok) {
            // Filter out the deleted instance from the state immediately
            const updatedInstances = instances.filter(instance => instance.id !== id);
            console.log("Updated instances after deletion:", updatedInstances); // Log updated instances
            setInstances([...updatedInstances]); // Ensure we're passing a new array to setInstances
            alert("Instance deleted successfully.");
            setSelectedInstance(null); // Clear selected instance after deletion if it was selected
        } else {
