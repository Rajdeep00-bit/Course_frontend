...
function App() {
  ...
  const handleCourseCreated = (newCourse) => {
    setCourses((prevCourses) => [...prevCourses, newCourse]);  // Append the new course to the current courses array
  };
  ...
  const deleteCourse = (id) => {
    ...
    fetch(`http://127.0.0.1:8000/api/courses/${id}/`, {
      ...
      .then((response) => {
        ...
        setCourses((prevCourses) => prevCourses.filter((course) => course.id !== id)); // Update state after deletion
