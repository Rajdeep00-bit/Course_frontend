...
function App() {
  ...
  // Handle course creation (triggered by CourseForm)
  const handleCourseCreated = () => {
    fetchCourses();  // Refresh course list immediately after course creation
  };
  ...
  const deleteCourse = (id) => {
    ...
    fetch(`http://127.0.0.1:8000/api/courses/${id}/`, {  // Ensure trailing slash
      ...
      .then((response) => {
        if (response.ok) {
          fetchCourses();  // Refresh course list immediately after deletion
  ...
  useEffect(() => {
    fetchCourses();  // Fetches courses once when the component mounts
  }, []);
