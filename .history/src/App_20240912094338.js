...
function App() {
  ...
  // Handle course creation: add the new course to the existing courses array
  const handleCourseCreated = (newCourse) => {
    console.log('Course created:', newCourse);  // Debug log

    // Try refetching the courses again after creating a new one
    fetchCourses();  // Refetch courses after creating a new course

    // Alternatively, you can manually append the course if the API returns the course data correctly
  ...
  const deleteCourse = (id) => {
    ...
    fetch(`http://127.0.0.1:8000/api/courses/${id}/`, {
      ...
      .then((response) => {
        ...
        // Refetch courses after deleting a course to ensure the list is updated
        fetchCourses();
