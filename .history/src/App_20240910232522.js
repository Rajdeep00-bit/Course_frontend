...
import CourseInstanceDetails from './components/CourseInstanceDetails'; // Importing the course instance details
...
function App() {
  ...
  const viewInstanceDetails = (id) => {
    fetch(`http://127.0.0.1:8000/api/instances/${year}/${semester}/${id}`)
      .then(response => response.json())
      .then(data => {
        setSelectedInstance(data); // Set selected instance to display its details
      })
      .catch((error) => {
  ...
  const deleteInstance = (id) => {
    ...
    .then((response) => {
      if (response.ok) {
        fetchInstances();  // Refresh the instance list after successful deletion
