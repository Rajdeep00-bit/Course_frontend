...
function App() {
  ...
  // Handle course creation - pass it to CourseForm as a callback
  const handleCourseCreated = () => {
    fetchCourses();  // Immediately refresh the course list after creation
  };
  ...
  const deleteCourse = (id) => {
    ...
    fetch(`http://127.0.0.1:8000/api/courses/${id}/`, {  // Ensure trailing slash
      ...
      .then((response) => {
        if (response.ok) {
          fetchCourses();  // Refresh the course list immediately
