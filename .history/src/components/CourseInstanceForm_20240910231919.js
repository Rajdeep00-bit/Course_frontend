...
function CourseInstanceForm({ onInstanceCreated, year, semester, setYear, setSemester, fetchNewCourse, setFetchNewCourse, fetchInstances }) {
  ...
  const handleSubmit = (event) => {
    ...
    fetch('http://127.0.0.1:8000/api/instances', {
      ...
      .then(data => {
        ...
        setSemester('');     // Reset semester field

        onInstanceCreated();  // Callback to refresh other components if needed
        fetchInstances();     // Refresh the course instance list
      })
      .catch((error) => {
