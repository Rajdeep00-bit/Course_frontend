...
function CourseList() {
  ...
  const createCourse = (newCourse) => {
    fetch('http://127.0.0.1:8000/api/courses/', {
      ...
      .then((createdCourse) => {
        setCourses((prevCourses) => [...prevCourses, createdCourse]);  // Add the newly created course to the state
      })
      .catch((error) => {
  ...
  const deleteCourse = (id) => {
    ...
    fetch(`http://127.0.0.1:8000/api/courses/${id}/`, {
      ...
      .then((response) => {
        if (response.ok) {
          setCourses((prevCourses) => prevCourses.filter(course => course.id !== id));  // Remove the deleted course from the state
