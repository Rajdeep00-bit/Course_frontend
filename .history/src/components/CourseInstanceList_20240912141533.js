...
function CourseInstanceList({ courses }) {
  ...
  const handleDeleteInstance = (id, year, semester) => {
    ...
    fetch(deleteUrl, {
      ...
      .then((response) => {
        ...
        fetchInstances(); // Fetch the updated list
      })
      .catch((error) => {
  ...
  // Find the course title for each instance
  const getCourseDetails = (courseId) => {
    const course = courses.find((c) => c.id === courseId);
    if (!course) {
      console.error(`Course not found for courseId: ${courseId}`);
    }
    return course ? { title: course.title } : { title: 'Unknown Course' };
  };
        ...
        <table>
          ...
          <tbody>
            {instances.map((instance) => {
              const { title } = getCourseDetails(instance.course);
              console.log(`Instance: ${instance.id}, Course Title: ${title}`); // Debugging statement
              return (
                <tr key={instance.id}>
                  <td>{title}</td>
