...
function CourseInstanceForm({
  ...
  // Fetch courses if not passed as props or when triggered by fetchNewCourse state
  useEffect(() => {
    if (!propCourses) {
  ...
  const handleSubmit = (event) => {
    ...
    fetch('http://127.0.0.1:8000/api/instances/', {
      ...
      .then((newInstance) => {
        ...
        // Optionally trigger a course refetch if needed
        setFetchNewCourse(!fetchNewCourse);
      })
