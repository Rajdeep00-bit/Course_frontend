...
function CourseList() {
  ...
  // Create a new course and append it to the list
  const createCourse = (newCourse) => {
    ...
    fetch('http://127.0.0.1:8000/api/courses/', {
      ...
      .then((createdCourse) => {
        ...
        setCourses((prevCourses) => [...prevCourses, createdCourse]);  // Append the new course to the state
      })
      .catch((error) => {
  ...
  // Delete a course and remove it from the list
    ...
    fetch(`http://127.0.0.1:8000/api/courses/${id}/`, {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          // Update the state by removing the deleted course from the list
