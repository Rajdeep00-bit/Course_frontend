...
function CourseInstanceList({ instances, setInstances, year, semester }) {
  ...
  const fetchInstances = () => {
    ...
    fetch(`http://127.0.0.1:8000/api/instances/${year}/${semester}`)
      ...
      .then((data) => setInstances(data))  // Set the instances state with fetched data
      .catch((error) => {
        ...
        <tbody>
          {instances.length === 0 ? (
            ...
            instances.map((instance) => (
              <tr key={instance.id}>
                <td>{instance.course.title}</td> {/* Display Course Title */}
                <td>{`${instance.year}-${instance.semester}`}</td>
                <td>{instance.code}</td> {/* Display Course Code */}
                <td>
